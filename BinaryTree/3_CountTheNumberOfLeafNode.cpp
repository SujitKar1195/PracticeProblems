/*
    Author: Sujit Kar
    Date: 26/06/2023
    Topic: Count the number of leaf nodes
    Time Complexity: O(n)
...
Discuss:
    ...
    *Leaf Node means which node has no left child and no right child.

    Steps to solve:
        1.Starting from root node,Using Level Order Traversal search for those nodes which left and right node is NULL;
